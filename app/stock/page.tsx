...
import { encodeItemToParam } from './[num]/page';
...
function extractImageUrls(obj: AnyObj): string[] {
  ...
  for (const c of candidates) {
    if (!c) continue;
    if (Array.isArray(c)) c.forEach((v) => pushUrl(String(v)));
    else pushUrl(String(c));
  }
...
export default function StockListPage() {
  ...
  const [cart, setCart] = useState<AnyObj[]>(() => {
    try {
      return JSON.parse(localStorage.getItem('imgparts_cart') || '[]');
    } catch {
      return [];
    }
  });
  useEffect(() => {
    try {
      localStorage.setItem('imgparts_cart', JSON.stringify(cart));
    } catch {}
  }, [cart]);
  ...
  const fetchPage = useCallback(
    async (p: number) => {
      ...
      try {
        ...
        // 无更多判断：后端是否返回最后一页标记？若没有，用长度判断
        setHasMore(arr.length > 0);
      } catch {
        // 保底：错误时不中断，只是停止 hasMore，按钮仍可手点
        setHasMore(false);
      } finally {
        setLoading(false);
      }
  ...
  const filtered = useMemo(() => {
    return list.filter((x) => {
      const hitQ =
        ...
        x.oe?.toLowerCase().includes(q.toLowerCase()) ||
        x.brand?.toLowerCase().includes(q.toLowerCase()) ||
        x.model?.toLowerCase().includes(q.toLowerCase());
  ...
  const brandOptions = useMemo(
    () => Array.from(new Set(list.map((x) => x.brand).filter(Boolean))),
    [list]
  );
  const modelOptions = useMemo(
    () => Array.from(new Set(list.map((x) => x.model).filter(Boolean))),
    [list]
  );
  const yearOptions = useMemo(
    () => Array.from(new Set(list.map((x) => x.year).filter(Boolean))),
    [list]
  );
  ...
  const removeFromCart = (it: AnyObj) => {
    setCart((old) => old.filter((x) => (x.oe || x.id) !== (it.oe || it.id)));
  };

  const setQty = (it: AnyObj, n: number) => {
    setCart((old) => {
      const copy = old.map((x) => ({ ...x }));
      const idx = copy.findIndex((x) => (x.oe || x.id) === (it.oe || it.id));
      if (idx >= 0) {
        copy[idx].qty = Math.max(1, n);
      }
      return copy;
    });
  };
    ...
    <main className="container mx-auto p-4">
      ...
      <div className="mb-6 flex items-center justify-between">
        ...
        <nav className="space-x-4 text-sm">
          ...
          <Link href="/stock?oe=1" className="hover:underline">
            OE 搜索
          </Link>
          <button
          ...
          >
            <div className="w-40 shrink-0">
              {it.imageUrls?.length ? (
                ...
                <div className="flex h-28 w-40 items-center justify-center rounded-md border text-gray-400">
                  No Image
                </div>
              )}
            </div>
            <div className="min-w-0 flex-1">
              <Link
                href={`/stock/${encodeURIComponent(it.sn || it.id || it.oe || '')}?d=${encodeURIComponent(
                  encodeItemToParam(it._raw || it)
                )}`}
              ...
              {!!it.oe && (
                <div className="mt-1 text-sm text-gray-600">OE: {it.oe}</div>
              )}
            ...
            <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
              ...
              <div>
                    ...
                    <div key={(it.oe || it.id) + '-cart'} className="flex gap-3 rounded-lg border p-3">
                      <div className="w-20 shrink-0">
                        {it.imageUrls?.length ? (
                          ...
                          <div className="flex h-16 w-20 items-center justify-center rounded border text-gray-400">
                            No Img
                          </div>
                        )}
                      </div>
                      <div className="min-w-0 flex-1">
                        ...
                        <div className="mt-2 flex items-center gap-2 text-sm">
                          ...
                          <button
                            ...
                            className="ml-3 rounded border px-2 text-red-600"
                          >
                            移除
                          </button>
                        </div>
                ...
                {cart.length > 0 && (
                  <div className="mt-3 text-right">
                    <button onClick={clearCart} className="rounded border px-3 py-1 hover:bg-gray-50">
                      清空购物车
                    </button>
                  </div>
                )}
              ...
              <div>
                <div className="mb-2 flex gap-2">
                  <button
                    onClick={() => setMode('B2C')}
                    className={`rounded-lg px-3 py-1 ${mode === 'B2C' ? 'bg-blue-600 text-white' : 'border'}`}
                  >
                    B2C（个人）
                  </button>
                  <button
                    onClick={() => setMode('B2B')}
                    className={`rounded-lg px-3 py-1 ${mode === 'B2B' ? 'bg-blue-600 text-white' : 'border'}`}
                  >
                    B2B（公司）
                  </button>
                </div>
