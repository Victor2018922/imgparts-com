...
function extractImages(fromObj: AnyObj | null, urlImage?: string): string[] {
  ...
  if (fromObj) {
    ...
    Object.keys(fromObj).forEach((k) => { if (/^(img|image|pic|photo)\d*$/i.test(k)) push(String(fromObj[k] || '')); });
...
function useCart() {
  ...
  const add = useCallback((it: Omit<CartItem, 'qty'>, qty = 1) => {
    setItems((prev) => {
      ...
      if (i > -1) { const next = [...prev]; next[i] = { ...next[i], qty: Math.min(999, next[i].qty + qty) }; return next; }
...
function CartButton({ cart }: { cart: ReturnType<typeof useCart> }) {
  return (
    <button onClick={() => cart.setOpen(true)} className="fixed z-40 right-6 bottom-6 rounded-full bg-emerald-600 text-white px-4 py-3 shadow-lg hover:bg-emerald-500">
...
function CartDrawer({ cart }: { cart: ReturnType<typeof useCart> }) {
  ...
  const checkout = () => { if (cart.items.length === 0) return; const id = 'IP' + String(Date.now()).slice(-10); setDone({ id, total: cart.total }); cart.clear(); };
  return (
    <>
      <div className={`fixed z-50 top-0 right-0 h-full w-[360px] bg-white shadow-2xl transition-transform duration-200 ${cart.open ? 'translate-x-0' : 'translate-x-full'}`}>
        ...
        <div className="p-4 space-y-3 overflow-auto h-[calc(100%-170px)]">
          {cart.items.length === 0 ? <div className="text-slate-400 text-sm">购物车是空的～</div> : cart.items.map((it) => (
            <div key={it.num} className="flex gap-3 items-center">
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="mt-1 flex items-center gap-2">
                  ...
                  <input className="w-12 text-center border rounded py-0.5" value={it.qty} onChange={(e) => cart.setQty(it.num, Number(e.target.value || 1))}/>
                  <button className="px-2 border rounded" onClick={() => cart.setQty(it.num, it.qty + 1)}>+</button>
                </div>
              </div>
...
export default function StockDetailPage() {
    ...
    const run = async () => {
      if (navIdx >= 0) {
        try {
          ...
          const found = list.find((x) => String(pick(x, ['num','Num','code','partNo'], '')).toLowerCase() === String(numParam).toLowerCase());
  ...
  const gotoBy = (step: number) => {
    ...
    setNavIdx(nextIdx);
    setMeta({ ...it });
    router.replace(`/stock/${encodeURIComponent(numNo)}?title=${encodeURIComponent(_title)}&oe=${encodeURIComponent(_oe)}&brand=${encodeURIComponent(_brand)}&price=${_price}&image=${encodeURIComponent(_image)}&idx=${nextIdx}`);
  };
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div>
          ...
          {!!imgs.length && (
            <div className="mt-3 flex gap-2 overflow-auto pb-1">
              {imgs.map((src, i) => (
                <button key={src + i} onClick={() => setCur(i)}
                  className={`shrink-0 w-24 h-20 rounded border ${i === cur ? 'border-emerald-600' : 'border-slate-200'} bg-white`} title={`图 ${i + 1}`}>
        ...
        <div>
          ...
          <div className="mt-4 flex items-center gap-3">
            <button onClick={addToCart} className="bg-emerald-600 text-white rounded px-4 py-2 hover:bg-emerald-500">加入购物车</button>
            <button onClick={() => gotoBy(-1)} className="border rounded px-4 py-2 disabled:opacity-40" disabled={navIdx <= 0}>上一条</button>
            <button onClick={() => gotoBy(+1)} className="border rounded px-4 py-2 disabled:opacity-40" disabled={navIdx < 0 || navIdx >= pageList.length - 1}>下一条</button>
          </div>
        </div>
      </div>
